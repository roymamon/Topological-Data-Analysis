...
points = pd.read_csv("data_B.csv") 

diagrams = ripser(points.values)['dgms']

plt.figure(figsize=(6, 6))
plt.scatter(points["x"], points["y"], s=10)
plt.title("Point Cloud for data_B.csv")
plt.axis("equal")
plt.grid(True)
plt.savefig("data_B_pointcloud.png", dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig("data_B_persistence_diagram.png", dpi=300)
plt.show()